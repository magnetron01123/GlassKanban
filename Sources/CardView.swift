...
struct CardView: View {
    ...
    private var strikeLine: some View {
        if card.status == .done, !isRenaming {
            CardStrikeLine(progress: strikeProgress)
        }
    }

    /// Shared with the menu bar tray — see `CardParts`.
    private var titleText: Text { CardParts.titleText(for: card) }
    ...
    private var listStripe: some View {
        CardListStripe(card: card, isSingleLine: density.isSingleLine)
    }
    ...
    /// All three shared with the menu bar tray — see `CardParts`.
    private var fullBadge: CardParts.BadgeInfo? { CardParts.fullBadge(for: card) }
    private var compactBadge: CardParts.BadgeInfo? { CardParts.compactBadge(for: card) }
    private func badgeView(_ info: CardParts.BadgeInfo) -> some View { CardBadgeView(info: info) }
